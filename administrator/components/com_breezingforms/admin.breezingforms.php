...
function addComponentMenu($row, $parent, $copy = false)
{
    ...
    if ($result) {
        ...
        return _ff_query(
            ...
            "1, " . intval($result) . ", 1, 'media/com_breezingforms/images/site/$row->img',( Select mlftrgt From (Select max(mlft.rgt)+1 As mlftrgt From #__menu As mlft) As tbone ),( Select mrgtrgt From (Select max(mrgt.rgt)+2 As mrgtrgt From #__menu As mrgt) As filet )" .
            ")",
            true
        );
    } else {
