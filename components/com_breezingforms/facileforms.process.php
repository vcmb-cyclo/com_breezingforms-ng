...
class HTML_facileFormsProcessor
{
    ...
    function loadBuiltins(&$library)
    {
        ...
        $code .= 'if(error != "" && document.getElementById(\'ff_capimgValue\')){
                 document.getElementById(\'ff_capimgValue\').src = \'' . Uri::root(true) . ($this->app->isClient('administrator') ? '/administrator' : '') . '/media/com_breezingforms/images/site/captcha/securimage_show.php?bfMathRandom=\' + Math.random();
            ...
            for ($i = 0; $i < $this->rowcount; $i++) {
                ...
                switch ($row->type) {
                    ...
                    case 'Tooltip':
                        ...
                        switch ($row->flag1) {
                            case 0:
                                $url = $ff_mossite . '/media/com_breezingforms/images/site/tooltip.png';
                                break;
                            case 1:
                                $url = $ff_mossite . '/media/com_breezingforms/images/site/warning.png';
                                break;
                            default:
                                $url = $data1;
                        } // switch
                    ...
                    case 'Captcha':
                        if ($this->app->isClient('site')) {
                            $captcha_url = Uri::root(true) . '/media/com_breezingforms/images/site/captcha/securimage_show.php';
                        } else {
                            $captcha_url = Uri::root(true) . '/media/com_breezingforms/images/site/captcha/securimage_show.php';
                        }
                        echo indentc(1) . '<div id="ff_div' . $row->id . '" style="' . $attribs . '"' . $class1 . '>' . nlc();
    ...
    function submit()
    {
        ...
        if (!$halt) {
            ...
            for ($i = 0; $i < $this->rowcount; $i++) {
                $row = $this->rows[$i];
                if ($row->type == "Captcha") {
                    require_once(JPATH_SITE . '/media/com_breezingforms/images/site/captcha/securimage.php');
...
// ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        var handler = StripeCheckout.configure({
            ...
            image: ' . json_encode(Uri::root() . 'media/com_breezingforms/images/site/icon_card.png') . ',
